...
public class ArtWorkE2ETestCase { 
 ...
 //String unique = "Sara-" + UUID.randomUUID().toString().substring(0, 8);
  ...
  public void testDeleteArtist_RemovesRowFromList() throws Exception {
      ...
      // 1) Prepare test data through REST API
      String id = postArtistJson("Delete", "Painting", 2);

      // 2) Open the real UI
      driver.get(baseUrl + "/artists");

      // 3) Verify the artist is visible before deleting
      ...
      // 4) Click the Delete link for that specific artist
      driver.findElement(
          By.xpath("//a[contains(@href,'artists/delete/" + id + "')]")
      ).click();

      // 5) Verify the artist is gone from the list
      wait.until(ExpectedConditions.urlContains("/artists"));
...
public void testHighestScoreArtists_ShowsOnlyTopScorers() throws Exception {
    ...
    //String topArtist1 = "Top-" + UUID.randomUUID().toString().substring(0, 8);
    //String topArtist2 = "Top-" + UUID.randomUUID().toString().substring(0, 8);
    //String lowArtist = "Low-" + UUID.randomUUID().toString().substring(0, 8);
    ...
    assertThat(tableText).contains("Top1", "Oil", "3");
    assertThat(tableText).contains("Top2", "Watercolor", "3");
    assertThat(tableText).doesNotContain("Low", "Sketch");
}
